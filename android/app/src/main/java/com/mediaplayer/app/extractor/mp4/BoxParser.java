 ...
 */
package com.mediaplayer.app.extractor.mp4;
...
public final class BoxParser {
  ...
  public static Metadata parseMdtaFromMeta(Mp4Box.ContainerBox meta) {
     LeafBox hdlrAtom = meta.getLeafBoxOfType(Mp4Box.TYPE_hdlr);
     LeafBox keysAtom = meta.getLeafBoxOfType(Mp4Box.TYPE_keys);
     LeafBox ilstAtom = meta.getLeafBoxOfType(Mp4Box.TYPE_ilst);
    if (hdlrAtom == null
        || keysAtom == null
        || ilstAtom == null
  ...
  public static Track parseTrak(
    ...
    if (!ignoreEditLists) {
      ...
      if (edtsAtom != null) {
         Pair<long[], long[]> edtsData = parseEdts(edtsAtom);
        if (edtsData != null) {
  ...
  public static TrackSampleTable parseStbl(
    ...
    SampleSizeBox sampleSizeBox;
     LeafBox stszAtom = stblBox.getLeafBoxOfType(Mp4Box.TYPE_stsz);
    if (stszAtom != null) {
      sampleSizeBox = new StszSampleSizeBox(stszAtom, track.format);
    } else {
       LeafBox stz2Atom = stblBox.getLeafBoxOfType(Mp4Box.TYPE_stz2);
      if (stz2Atom == null) {
    ...
    boolean chunkOffsetsAreLongs = false;
     LeafBox chunkOffsetsAtom = stblBox.getLeafBoxOfType(Mp4Box.TYPE_stco);
    ...
    // Entries are the indices of samples that are synchronization samples.
     LeafBox stssAtom = stblBox.getLeafBoxOfType(Mp4Box.TYPE_stss);
     ParsableByteArray stss = stssAtom != null ? stssAtom.data : null;
    // Entries are (number of samples, timestamp offset).
     LeafBox cttsAtom = stblBox.getLeafBoxOfType(Mp4Box.TYPE_ctts);
  ...
  private static @C.TrackType int getTrackTypeForHdlr(int hdlr) {
    android.util.Log.e("Av3aDebug", "getTrackTypeForHdlr called with hdlr=" + Integer.toHexString(hdlr));
    if (hdlr == TYPE_soun) {
      ...
      android.util.Log.e("Av3aDebug", "getTrackTypeForHdlr returned UNKNOWN for hdlr=" + Integer.toHexString(hdlr));
  ...
  private static Pair<long[], long[]> parseEdts(Mp4Box.ContainerBox edtsAtom) {
     LeafBox elstAtom = edtsAtom.getLeafBoxOfType(Mp4Box.TYPE_elst);
    if (elstAtom == null) {
      return null;
    }
  ...
  private static void parseAudioSampleEntry(
    ...
    } else if (atomType == 0x61763361) { // 'av3a'
      mimeType = "audio/av3a";
      android.util.Log.e("Av3aDebug", "BINGO! BoxParser found 0x61763361 (av3a)! Set mimeType to audio/av3a");
    }
    ...
    while (childPosition - position < size) {
      ...
      if (childAtomType == Mp4Box.TYPE_mhaC) {
        ...
        initializationData = java.util.Collections.singletonList(headerBytes);
